...
const app = {
  consts: {
    ...
    mediasrc: "/data/nam_le-sources.json", // json list of all frames
    startDate: new Date("2022-05-24T00:00:00"), // date of issue #0 - countdown point
  },
  datePickerOptions: {
  ...
  announcements: {
    ...
    highContrastModeAdded: 1, // added new option in settings
  },
  stats: {
  ...
  html: {
    ...
    game: {
      ...
      guesses: $("div.user-guess-history"),
      container: $("div#game"),
    },
    gameinfo: {
      ...
      name: $("div#frameGame"),
      link: $("a#frameVod"),
      container: $("div.user-extended-info"),
    },
    hardMode: {
      // labels and counters for hard mode
      hmAttemptCounter: $("#hardModeCounter"),
      container: $("div.user-guess-hard-mode"),
    },
    controls: {
      ...
      container: $("div.user-guess-input"),
    },
    buttons: {
      // top panel buttons
      share: $("#shareButton"),
  ...
  aux: {
    ...
    renderNotifications: () => {
      ...
      // if player has good stats, offer to increase game difficulty
      if (!app.announcements.hardModeOffer && app.stats.games > 5 && app.stats.avg < 7) {
        ...
        tipParent.popover({
          ...
          trigger: "manual",
          html: true,
          content: `<img alt="nam" class="inline-smol" src="/pics/nam_le/nam.png" width="16">&nbsp;&nbsp;Game too easy?<br>Try <b>♨️ Hard mode</b>! `,
        });
    ...
    renderAdvice: (advice = "") => {
      if (advice) {
        ...
        const tipParent = app.html.controls.inputDate.is(":visible") ? app.html.controls.inputDate : app.html.controls.inputDate.parent();
        ...
        app.html.controls.inputDate.attr("data-bs-content", "");
        app.html.controls.inputDate.parent().attr("data-bs-content", "");
    ...
    fillSrcInfo: () => {
      ...
      // build link
      const linksrc = String(app.src.src); // this can be used to mark pictures from the same vod!
      if (!linksrc) {
        app.html.gameinfo.link.hide();
      } else {
        app.html.gameinfo.link.prop("href", linksrc);
    ...
    load: () => {
      // load field settings
      try {
        let sets = localStorage.getItem("nam_le-settings");
        sets = JSON.parse(sets);
        Object.keys(app.settings).forEach((k) => {
          if (sets.hasOwnProperty(k)) app.settings[k] = sets[k];
        });
      } catch (e) {
        console.warn("Failed to load game settings", e);
        // fallback to settings from forsenle
        try {
          ...
          console.info("Using fallback settings from forsenle.");
        } catch (e) {
          console.warn("Failed to load fallback game settings", e);
        }
      }
      // load announcements
      try {
        let announcements = localStorage.getItem("nam_le-announcements");
        announcements = JSON.parse(announcements);
        Object.keys(app.announcements).forEach((k) => {
          if (announcements.hasOwnProperty(k)) app.announcements[k] = announcements[k];
        });
      } catch (e) {
        console.warn("Failed to load game settings", e);
      }
      // load field state
      try {
        let field = localStorage.getItem("nam_le-state");
        field = JSON.parse(field);
        Object.keys(app.game).forEach((k) => {
          if (field.hasOwnProperty(k)) app.game[k] = field[k];
        });
      } catch (e) {
      ...
      // load stats
      try {
        let stats = localStorage.getItem("nam_le-stats");
    ...
    save: () => {
      localStorage.setItem("nam_le-state", JSON.stringify(app.game));
      localStorage.setItem("nam_le-stats", JSON.stringify(app.stats));
      localStorage.setItem("nam_le-settings", JSON.stringify(app.settings));
      localStorage.setItem("nam_le-announcements", JSON.stringify(app.announcements));
    },

    // action: create a fancy string with emojis and send it to clipboard or mobile "share"
    share: () => {
      try {
        let str = "NaM______________________le ";
        if (app.game.victory) {
          str += `#${app.currentIssue + 1} - ${app.game.victory > 0 ? "👤 in " + app.game.tries.length : "🙅"}${app.settings.hardMode ? " ♨️" : ""}`;
          let lastRow = "",
        ...
        let winrate = Math.round((100 * app.stats.wins) / Math.max(app.stats.games, 1));
        ...
        if (!app.game.victory && app.stats.games < 1) str = "🧔 NaM______________________le\n" + window.location.origin + window.location.pathname;
    ...
    victory: (delay = 0) => {
      ...
      if (app.game.victory !== 0) {
        ...
        // modal title
        app.html.stats.title.text("👤 Game over!");
      ...
      if (app.game.victory || app.stats.games > 0) {
        ...
        // fill stats:
        Object.keys(app.stats).forEach((stat) => {
          switch (stat) {
            case "avg": // avgtimes stat is a float, needs some love:
              app.html.stats.avgtimes.text(app.stats[stat].toFixed(2));
              break;
            case "wins": // calculate winrate
              let winrate = (100 * app.stats.wins) / Math.max(app.stats.games, 1);
              app.html.stats.winrate.text(Math.round(winrate) + "%");
              break;
            case "min":
            case "max": // min-max can look weird if user lost a hard mode game as first run
              app.html.stats[stat].text(app.stats[stat] < 0 ? "-" : app.stats[stat]);
              break;
            default:
              // each html node should have appropriate id for the stat
              if (!app.html.stats.hasOwnProperty(stat)) return;
      ...
      setTimeout(() => {
        ...
        if (app.game.victory > 0 && delay > 0) {
          // win = confetti
          app.aux.sprinkleSomeLove(delay / 2);
        }
...
window.onload = function () {
      ...
      app.html.game.frame.on("error", (event) => {
        ...
        showError(
          "Stream image could not be loaded!<br><br>Try refreshing the page. If this happens again, please " +
            `<a href="mailto:admin@g7eternal.com?subject=NaM_le%20not%20working%20(missing%20image)">notify me</a> about the problem.`
        );
        // flavor:
        app.html.loader.img.prop("src", "/pics/nam_le/cry.gif");
        app.html.loader.text.html("Something broke!<br>Please try again later");
    ...
    .catch((e) => {
      ...
      showError(
        "An error occured while retrieving list of stream frames.<br>Try refreshing the page. If this happens again, please " +
          `<a href="mailto:admin@g7eternal.com?subject=NaM_le%20not%20working%20(JSON parse)&body=Error:%20${n}:%20${t}%0D%0A${s}">notify me</a> about the problem.` +
  ...
  app.html.controls.submitBtn.on("click", (event) => {
    ...
    try {
      ...
      if (app.game.victory !== 0) {
        ...
        app.html.controls.container.hide();
        app.html.hardMode.container.hide();
        app.aux.victory(2000 + 1000 * (app.game.victory < 0));
      }
      app.aux.save();
      setTimeout(() => {
      ...
      // show popover
      app.aux.renderAdvice(String(e.message || e).replace(/\n/g, "<br>"));
  ...
  /*
    ...
    app.popovers = popoverTriggerList.map(function(element) {
